...
async def import_txt_cookie(page: Page, profile_name: str, cookie_path: str) -> bool:
    ...
    try:
        await ext_page.goto(EXT_URL, wait_until="domcontentloaded", timeout=60000)

        clear_button = ext_page.locator('css=button#clear-all-data-btn').first
        await clear_button.wait_for(state="attached", timeout=15000)
        await clear_button.wait_for(state="visible", timeout=15000)
